# Wishtrip · Kyoto & Shillong prototype

Wishtrip is a focused trip-planning prototype for two deeply modeled destinations — **Kyoto, Japan** and **Shillong, India (Meghalaya)** — instead of shallow, broad coverage. It pairs a responsive React/Vite/TypeScript interface with a FastAPI/Pydantic API and a deterministic, explainable planning engine. An optional Claude layer only polishes the explanation copy; it never invents or changes activities, prices, or timings.
...
backend/   FastAPI + Pydantic
   app/data.py       curated activity data for both destinations, keyed by destination
...
6. **Seasonal notes**: a short, destination- and month-specific note (e.g. cherry blossom crowding in April Kyoto, monsoon trail conditions in July Shillong) is attached based on the trip's start month.
7. **Travel time**: estimated as straight-line distance between coordinates × 12 minutes/km — a deliberate simplification, surfaced in the UI as an estimate rather than presented as routed, road-accurate time.

## Data model and sources

Each `Activity` (see `backend/app/models.py`) has: id, name, category, description, tags, neighborhood, coordinates, duration, price per person, opening hours/weekdays, best time of day, indoor/accessible flags, dietary options, a popularity score, and an image.

- **Kyoto** (14 activities, USD): well-known points of interest (Fushimi Inari, Kiyomizu-dera, Arashiyama, Nishiki Market, etc.).
- **Shillong** (14 activities, INR): well-known Meghalaya points of interest (Umiam Lake, Elephant Falls, the Nongriat living root bridge, Laitlum Canyons, Police Bazar, Mawlynnong, etc.).

Place names, neighborhoods, and approximate coordinates are based on publicly known points of interest (the kind that appear on OpenStreetMap and general travel references). **Prices, durations, opening hours, and popularity scores are reasonable synthetic estimates for prototype purposes** — they are not scraped from a live booking source, and should not be treated as current, bookable prices. This is stated here rather than presented as fact.

## Constraints, assumptions, and known limitations

- **Two destinations only**, each deeply modeled with ~14 hand-curated activities. Broader city coverage was explicitly deprioritized in favor of itinerary quality on a smaller, well-understood dataset.
- **Travel time is a straight-line estimate** (× 12 min/km), not a routed, road- or terrain-aware calculation — it will understate travel time in places with rivers, hills, or indirect roads (this matters especially for Shillong's hill terrain).
...
**1. Kyoto, food-focused couple, balanced pace, $1500**
`origin_city: Mumbai, destination: Kyoto, interests: [food, culture], pace: balanced, budget: 1500, travelers: 2`
→ Days lean heavily on Nishiki Market, tea ceremony, Pontocho/Shigetsu dinners; cost breakdown shows Food as the largest slice.

**2. Kyoto, same dates, outdoors + relaxed pace, half the budget**
`interests: [outdoors, wellness], pace: relaxed, budget: 700`
→ Fewer stops per day (relaxed caps daily hours lower), Arashiyama/Philosopher's Path/Kurama onsen surface instead of paid workshops, and the budget guardrail excludes pricier dinners once the target is met — a visibly different, cheaper itinerary from the same destination and dates.

**3. Shillong, adventure-leaning friends group, packed pace, ₹15,000**
`origin_city: Guwahati, destination: Shillong, currency: INR, traveller_type: friends, interests: [outdoors, adventure], pace: packed, budget: 15000`
→ Prioritizes Living Root Bridge trek, Mawsmai Caves, Laitlum Canyons, and waterfalls; packed pace allows more stops per day; a November trip date surfaces the "clear, dry trekking weather" seasonal note, while a July date would surface the monsoon-trail warning instead.

These three requests use the same engine and data but produce meaningfully different day-by-day plans because every input (interests, pace, budget, destination, and even trip month) directly changes the filtering, scoring, or seasonal messaging — not just cosmetic text.

## What I'd improve with more time and better data

- Route travel time with a real routing engine (e.g. self-hosted OSRM) instead of straight-line distance, especially for Shillong's hill terrain.
- Expand each destination's activity count and source prices from a maintained public dataset rather than synthetic estimates.
