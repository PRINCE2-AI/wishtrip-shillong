...
Each `Activity` (see `backend/app/models.py`) has: id, name, category, description, tags, neighborhood, coordinates, duration, price per person (INR), opening hours/weekdays, best time of day, indoor/accessible flags, dietary options, a popularity score, an image, and three content fields aimed at making the itinerary feel like local knowledge rather than a database dump: `insider_tip` (a specific best-time-to-visit reason, not just "morning/afternoon/evening"), `local_story` (a short cultural or historical note — including, where it genuinely exists, real Khasi folklore like the Nohkalikai Falls legend), and optional `local_name` / `etiquette_note` / `myth_fact` fields (e.g. correcting the common "Cherrapunji is the wettest place on Earth" claim, a record that now belongs to nearby Mawsynram).
...
Place names, neighborhoods, and approximate coordinates are based on publicly known points of interest (the kind that appear on OpenStreetMap and general travel references for Shillong and Cherrapunji). **Prices, durations, opening hours, and popularity scores are reasonable synthetic estimates for prototype purposes** — they are not scraped from a live booking source, and should not be treated as current, bookable prices. The insider tips, local stories, and etiquette notes reflect commonly documented Khasi cultural context and general travel-writing knowledge, not a single verified source — they're included as reasonable, honestly-labeled background, not fact-checked trivia. This is stated here rather than presented as fact.
